...
# Plot --------------------------------------------------------------------
independence %>%
  ggplot(aes(x = decade, color = independence_from)) +
  geom_freqpoly(binwidth = 10,
                show.legend = FALSE) +
  ggbrookings::scale_color_brookings(palette = 'categorical', reverse = TRUE) +
  ...
  labs(
    title = "**The U.K. took its time, didn't it?**",
...
ggsave(
  here::here('figures', '02-independence-days.png'),
  width = 85 * (14 / 5),
  height = 53 * (14 / 5),
  units = 'mm',
  dpi = 300
